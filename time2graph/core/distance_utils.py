...
def softmax(x):
    ...
    # def __errcall(type, msg):
    #     Debugger.info_print(msg='RuntimeWarning: {}-{}, {}'.format(type, msg, x))
    # np.seterrcall(__errcall)
...
def parameterized_gw_npy(x, y, w, warp):
    ...
    '''
    TODO
    这里是可以改进的点，原文通过求离shapelet最近的片段的距离来代表当前shapelet和序列的距离
    '''
...
def pattern_distance_no_timing(pattern, time_series, warp, measurement):
    ...
    return np.sum(softmax_1d(-ret) * ret)
